...
describe('Context error paths', () => {
    test('Context with empty query', async () => {
        await expect(assistant.context({ query: '' })).rejects.toThrow('You must pass an object with required properties (`query`, or `messages`) to retrieve context snippets.');
    });
    test('Context with nonexistent assistant', async () => {
        await expect(pinecone.Assistant({ name: 'nonexistent' }).context({
