...
describe('getFetch', () => {
    ...
    describe('retry logic with maxRetries = 0', () => {
        ...
        test('should make exactly 1 attempt when maxRetries is 0 and request returns 5xx', async () => {
            ...
            const fetchFn = (0, fetch_1.getFetch)(config);
            await expect(fetchFn('https://example.com', {})).rejects.toThrow(errors_1.PineconeMaxRetriesExceededError);
            expect(customFetch).toHaveBeenCalledTimes(1);
        });
        test('should make exactly 1 attempt when maxRetries is 0 and request throws retryable error', async () => {
            ...
            retryableError.name = 'PineconeUnavailableError';
    ...
    describe('retry logic with maxRetries = 1', () => {
        ...
        test('should make up to 2 attempts when maxRetries is 1 and request returns 5xx', async () => {
            ...
            const fetchFn = (0, fetch_1.getFetch)(config);
            const promise = expect(fetchFn('https://example.com', {})).rejects.toThrow(errors_1.PineconeMaxRetriesExceededError);
    ...
    describe('retry logic with default maxRetries (3)', () => {
        ...
        test('should retry on PineconeInternalServerError', async () => {
            ...
            retryableError.name = 'PineconeInternalServerError';
            const customFetch = jest
                .fn()
                .mockRejectedValueOnce(retryableError)
                .mockResolvedValueOnce({ status: 200, ok: true });
        ...
        test('should not retry non-retryable errors', async () => {
            ...
            const fetchFn = (0, fetch_1.getFetch)(config);
            const promise = expect(fetchFn('https://example.com', {})).rejects.toThrow('Connection refused');
            await jest.runAllTimersAsync();
            await promise;
            expect(customFetch).toHaveBeenCalledTimes(1);
        });
    ...
    describe('edge cases', () => {
        ...
        test('should pass through request init options', async () => {
            ...
            expect(customFetch).toHaveBeenCalledWith('https://example.com', requestInit);
        });
