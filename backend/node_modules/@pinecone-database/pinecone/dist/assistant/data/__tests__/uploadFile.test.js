...
const buildMockFetchResponse = (mock, isSuccess, status, body) => mock.mockResolvedValue({
    ok: isSuccess,
    status: status,
...
describe('validation', () => {
    test('throws when called with no options', async () => {
        ...
        await expect(upload(undefined)).rejects.toThrow('You must pass an object with required properties');
    });
    test('throws when neither path nor file is provided', async () => {
        const upload = (0, uploadFile_1.uploadFile)(mockAssistantName, mockApiProvider, mockConfig);
        await expect(upload({})).rejects.toThrow('You must pass an object with required properties');
    });
    test('throws when only metadata is provided', async () => {
        const upload = (0, uploadFile_1.uploadFile)(mockAssistantName, mockApiProvider, mockConfig);
        await expect(upload({ metadata: { key: 'value' } })).rejects.toThrow('You must pass an object with required properties');
    });
    test('throws when path is empty string', async () => {
        const upload = (0, uploadFile_1.uploadFile)(mockAssistantName, mockApiProvider, mockConfig);
        await expect(upload({ path: '' })).rejects.toThrow('You must pass an object with required properties');
    });
    test('throws when file is provided without fileName', async () => {
        const upload = (0, uploadFile_1.uploadFile)(mockAssistantName, mockApiProvider, mockConfig);
        await expect(upload({ file: Buffer.from('data'), fileName: '' })).rejects.toThrow('`fileName` is required when uploading via `file`.');
    });
});
// ---------------------------------------------------------------------------
...
describe('path input', () => {
    ...
    beforeEach(() => {
        fs_1.default.promises.readFile.mockResolvedValue(mockFileContent);
        path_1.default.basename.mockReturnValue('test.txt');
        buildMockFetchResponse(mockRetryingFetch, true, 200, JSON.stringify(mockResponse));
    });
    test('reads file asynchronously from path', async () => {
        const upload = (0, uploadFile_1.uploadFile)(mockAssistantName, mockApiProvider, mockConfig);
        await upload({ path: 'test.txt' });
        expect(fs_1.default.promises.readFile).toHaveBeenCalledWith('test.txt');
    });
    ...
    test('sends FormData body', async () => {
        const upload = (0, uploadFile_1.uploadFile)(mockAssistantName, mockApiProvider, mockConfig);
        await upload({ path: 'test.txt' });
        expect(mockRetryingFetch).toHaveBeenCalledWith('https://prod-1-data.ke.pinecone.io/assistant/files/test-assistant', expect.objectContaining({
            method: 'POST',
    ...
    test('builds URL with metadata', async () => {
        ...
        expect(mockRetryingFetch).toHaveBeenCalledWith(`https://prod-1-data.ke.pinecone.io/assistant/files/test-assistant?metadata=${encodedMetadata}`, expect.anything());
    });
    test('builds URL with multimodal=true', async () => {
        const upload = (0, uploadFile_1.uploadFile)(mockAssistantName, mockApiProvider, mockConfig);
        await upload({ path: 'test.txt', multimodal: true });
        expect(mockRetryingFetch).toHaveBeenCalledWith('https://prod-1-data.ke.pinecone.io/assistant/files/test-assistant?multimodal=true', expect.anything());
    });
    test('builds URL with multimodal=false', async () => {
        const upload = (0, uploadFile_1.uploadFile)(mockAssistantName, mockApiProvider, mockConfig);
        await upload({ path: 'test.txt', multimodal: false });
        expect(mockRetryingFetch).toHaveBeenCalledWith('https://prod-1-data.ke.pinecone.io/assistant/files/test-assistant?multimodal=false', expect.anything());
    });
    test('builds URL with both metadata and multimodal', async () => {
        ...
        const encodedMetadata = encodeURIComponent(JSON.stringify(metadata));
        expect(mockRetryingFetch).toHaveBeenCalledWith(`https://prod-1-data.ke.pinecone.io/assistant/files/test-assistant?metadata=${encodedMetadata}&multimodal=true`, expect.anything());
    });
    test('includes required headers', async () => {
        const upload = (0, uploadFile_1.uploadFile)(mockAssistantName, mockApiProvider, mockConfig);
        await upload({ path: 'test.txt' });
        expect(mockRetryingFetch).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({
...
describe('ReadableStream input', () => {
    beforeEach(() => {
        buildMockFetchResponse(mockNonRetryingFetch, true, 200, JSON.stringify(mockResponse));
    });
    test('uses non-retrying fetch', async () => {
        const upload = (0, uploadFile_1.uploadFile)(mockAssistantName, mockApiProvider, mockConfig);
        const stream = stream_1.Readable.from(['pdf content']);
    ...
    test('sends a ReadableStream body (not FormData)', async () => {
        ...
        const stream = stream_1.Readable.from(['pdf content']);
        await upload({ file: stream, fileName: 'doc.pdf' });
        expect(mockNonRetryingFetch).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({
            method: 'POST',
    ...
    test('escapes special characters in fileName for Content-Disposition header', async () => {
        const upload = (0, uploadFile_1.uploadFile)(mockAssistantName, mockApiProvider, mockConfig);
        const stream = stream_1.Readable.from([Buffer.from('pdf content')]);
    ...
    test('builds URL with metadata', async () => {
        ...
        const encodedMetadata = encodeURIComponent(JSON.stringify(metadata));
        expect(mockNonRetryingFetch).toHaveBeenCalledWith(`https://prod-1-data.ke.pinecone.io/assistant/files/test-assistant?metadata=${encodedMetadata}`, expect.anything());
    });
});
