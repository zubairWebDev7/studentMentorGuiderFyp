...
const createIndexFromBackup = (api) => {
    return async (createIndexFromBackupOptions) => {
        if (!createIndexFromBackupOptions.backupId) {
            throw new errors_1.PineconeArgumentError('You must pass a non-empty string for `backupId` in order to create an index from backup');
        }
        else if (!createIndexFromBackupOptions.name) {
            throw new errors_1.PineconeArgumentError('You must pass a non-empty string for `name` in order to create an index from backup');
        }
