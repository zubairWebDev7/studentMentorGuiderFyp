...
describe('rerank', () => {
    test('Confirm throws error if no documents are passed', async () => {
        ...
        rerankCmd({ model: rerankModel, query: myQuery })).rejects.toThrow(new errors_1.PineconeArgumentError('You must pass at least one document to rerank'));
    });
    test('Confirm docs as list of strings is converted to list of objects with `text` key', async () => {
        ...
        expect(IA.rerank).toHaveBeenCalledWith(expect.objectContaining({ rerankRequest: expectedReq }));
    });
    ...
    test('Confirm error thrown if query is missing', async () => {
        ...
        const IA = setupRerankResponse();
        const rerankCmd = (0, rerank_1.rerank)(IA);
        await expect(rerankCmd({ model: rerankModel, query: myQuery, documents: myDocuments })).rejects.toThrow(new errors_1.PineconeArgumentError('You must pass a query to rerank'));
    });
    test('Confirm error thrown if model is missing', async () => {
        ...
        const IA = setupRerankResponse();
        const rerankCmd = (0, rerank_1.rerank)(IA);
        await expect(rerankCmd({ model: rerankModel, query: myQuery, documents: myDocuments })).rejects.toThrow(new errors_1.PineconeArgumentError('You must pass the name of a supported reranking model in order to rerank' +
