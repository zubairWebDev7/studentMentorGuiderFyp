...
describe('evaluate happy path', () => {
    test('evaluate with valid question, answer, and groundTruth', async () => {
        const result = await pinecone.evaluate({
...
describe('evaluate error paths', () => {
    test('evaluate with null options', async () => {
        ...
        await expect(pinecone.evaluate(null)).rejects.toThrow('You must pass an object with required properties (`question`, `answer`, `groundTruth`) to evaluate.');
    });
    test('evaluate with undefined options', async () => {
        // @ts-expect-error - invalid options
        await expect(pinecone.evaluate(undefined)).rejects.toThrow('You must pass an object with required properties (`question`, `answer`, `groundTruth`) to evaluate.');
    });
    test('evaluate with empty question', async () => {
        await expect(pinecone.evaluate({
            question: '',
            answer: 'Paris',
            groundTruth: 'Paris is the capital of France.',
        })).rejects.toThrow('Invalid input. Question, answer, and groundTruth must be non-empty strings.');
    });
