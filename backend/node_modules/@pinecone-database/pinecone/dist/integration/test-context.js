...
const getTestContext = async () => {
    ...
    if (!fixturesJson) {
        throw new Error('FIXTURES_JSON environment variable not set.\n\n' +
    ...
    try {
        ...
        throw new Error(`Failed to parse FIXTURES_JSON: ${error}\n` +
    ...
    if (!metadataFilter?.key || metadataFilter?.value === undefined) {
        throw new Error('FIXTURES_JSON missing serverlessIndex.metadataFilter with key and value');
    }
    const recordIds = data.serverlessIndex.recordIds || [];
    if (recordIds.length === 0) {
        throw new Error('FIXTURES_JSON missing serverlessIndex.recordIds');
    }
    // Create Pinecone client
    const client = new index_1.Pinecone({ apiKey });
    return {
        client,
        serverlessIndex: {
