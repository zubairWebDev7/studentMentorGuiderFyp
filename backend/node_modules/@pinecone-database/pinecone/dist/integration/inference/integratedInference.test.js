...
describe('Integrated Inference API tests', () => {
    ...
    beforeAll(async () => {
        ...
        indexName = (0, test_helpers_1.randomName)('int-inf');
    ...
    test('test upserting and searching records', async () => {
        ...
        await (0, test_helpers_1.assertWithRetries)(() => pinecone.index({ name: indexName }).describeIndexStats(), (stats) => {
            expect(stats.totalRecordCount).toBeGreaterThanOrEqual(8);
        }, 30000, // max wait 30s
        2000);
        await (0, test_helpers_1.assertWithRetries)(() => pinecone.index({ name: indexName }).searchRecords({
