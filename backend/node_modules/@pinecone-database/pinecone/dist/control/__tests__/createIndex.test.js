...
describe('createIndex', () => {
    ...
    test('Throw error if name, dimension, or spec are not passed', async () => {
        ...
        await expect(toThrow).rejects.toThrowError(errors_1.PineconeArgumentError);
        await expect(toThrow).rejects.toThrow('You must pass a `pods`, `serverless`, or `byoc` `spec` object in order to create an index.');
        // Missing dimension
        toThrow = async () => {
            // @ts-ignore
            await (0, createIndex_1.createIndex)(MIA)({
    ...
    describe('createIndex with BYOC spec', () => {
        ...
        test('throws error if BYOC environment is not provided', async () => {
            ...
            await expect(toThrow).rejects.toThrowError(errors_1.PineconeArgumentError);
            await expect(toThrow).rejects.toThrowError('You must pass an `environment` for the `CreateIndexByocSpec` object to create an index.');
        });
    });
    describe('createIndex readCapacity validation', () => {
        test('throws error for invalid readCapacity mode', async () => {
            ...
            await expect(toThrow).rejects.toThrowError(errors_1.PineconeArgumentError);
            await expect(toThrow).rejects.toThrowError(/Invalid read capacity mode.*Valid values are.*OnDemand.*Dedicated/i);
        });
        test('throws error for invalid dedicated nodeType', async () => {
            ...
            await expect(toThrow).rejects.toThrowError(errors_1.PineconeArgumentError);
            await expect(toThrow).rejects.toThrowError(/Invalid node type.*Valid values are.*b1.*t1/i);
        });
        test('throws error for missing manual config in dedicated mode', async () => {
            ...
            await expect(toThrow).rejects.toThrowError(errors_1.PineconeArgumentError);
            await expect(toThrow).rejects.toThrowError(/manual is required for dedicated mode/i);
        });
        test('throws error for invalid replicas value', async () => {
            ...
            await expect(toThrow).rejects.toThrowError(errors_1.PineconeArgumentError);
            await expect(toThrow).rejects.toThrowError(/replicas must be 0 or a positive integer/i);
        });
        test('throws error for invalid shards value', async () => {
            ...
            await expect(toThrow).rejects.toThrowError(errors_1.PineconeArgumentError);
            await expect(toThrow).rejects.toThrowError(/shards must be a positive integer/i);
        });
            ...
            const toThrow = async () => await (0, createIndex_1.createIndex)(MIA)({
                ...
                spec: {
                    serverless: {
                        ...
                        region: 'us-east-1',
                        readCapacity: {
                            mode: 'dedicated', // Testing runtime case-insensitive handling
                            manual: { replicas: 1, shards: 1 },
                        },
                    },
                },
            ...
            await (0, createIndex_1.createIndex)(MIA)({
                ...
                spec: {
                    serverless: {
                        ...
                        region: 'us-east-1',
                        readCapacity: {
                            mode: 'ondemand', // Testing runtime case-insensitive handling
                        },
                    },
                },
            });
